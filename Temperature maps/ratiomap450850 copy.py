...
def zoominupper(i, j, min, max, count, upperratio, lookup):
    mid = (min+max)/2
    count += 1
    if(count < 14):
        if(upperratio[i][j] > lookup[mid][1]):
            zoominupper(i,j,int(mid)-1, max, count, upperratio, lookup)
        elif(upperratio[i][j] < lookup[mid][1]):
            zoominupper(i,j,min, int(mid)+1, count, upperratio, lookup)
    else:    
        result = 1
        difference = 100
        for k in range(min, max):
...
def zoominlower(i, j, min, max, count, lowerratio, lookup):
    mid = (min+max)/2
    count += 1
    if(count < 14):
        if(lowerratio[i][j] > lookup[mid][1]):
            zoominlower(i,j,int(mid)-1, max, count,lowerratio, lookup)
        elif(lowerratio[i][j] < lookup[mid][1]):
            zoominlower(i,j,min, int(mid)+1, count, lowerratio, lookup)
    else:        
        result = 1
        difference = 100
        for k in range(min, max):                                               
...
def temperature(fits,var,beta,LOGIC):
    #Calculate dim numbers from ndftrace (TBC)
    dim = PARGET(fits,'dims','ndftrace')
    ...
    for i in range(0, row):
        for j in range(0, column):
            if ratio[i][j] > 0:
                ...
                """
                if(LOGIC == "TRUE"):
                    zoominupper(i,j,0, 99500, 0, upperratio, lookup)
                    zoominlower(i,j,0, 99500, 0, lowerratio, lookup) #call limits if applicable and calculate variance by difference of limits
...
#output new values (temperature from ratio) back to FITS file and save
    ...
    image.close()           
    """
    imvariance.flush() #Use if I want to produce variance maps
    imvariance.close()
    """
...
#Each map has a STDV noise level as taken from http://wiki.astro.ex.ac.uk/bin/view/JCMTGouldBelt/SCUBA-2IR1NoiseEstimates
#or calculated with noise_measurement_CS.py. 
#NOTE: This is section to later be updated to be run in the script. 
while True:
    if(map == 'south'):
        percent = 5
        STDV_south_450 = 0.0496759118911   #Jy/pixel
        STDV_south_850 = 0.0090694917334   #Jy/pixel
        sigma450 = STDV_south_450*snr
        sigma850 = STDV_south_850*snr
        var450 = (STDV_south_450**2.0)*((alpha450**2.0) + (beta450**2.0))*(2./3.)
        var850 = (STDV_south_850**2.0)*((alpha850**2.0) + (beta850**2.0))*(2./3.)
        break
    elif (map == 'mwc297'):
        percent = 5
        STDV_south_450 = 0.0164665781382   #Jy/pixel
        STDV_south_850 = 0.00218410360456   #Jy/pixel
        sigma450 = STDV_south_450*snr
        sigma850 = STDV_south_850*snr
        var450 = (STDV_south_450**2.0)*((alpha450**2.0) + (beta450**2.0))*(2./3.)
        var850 = (STDV_south_850**2.0)*((alpha850**2.0) + (beta850**2.0))*(2./3.)
        ...
        percent = 5
        STDV_south_450 = 0.0481830581482   #Jy/pixel
        STDV_south_850 = 0.0101566273883  #Jy/pixel
        sigma450 = STDV_south_450*snr
        sigma850 = STDV_south_850*snr
        var450 = (STDV_south_450**2.0)*((alpha450**2.0) + (beta450**2.0))*(2./3.)
        var850 = (STDV_south_850**2.0)*((alpha850**2.0) + (beta850**2.0))*(2./3.)
        break
    map = str(raw_input("Please enter the correct name of the map: ")) #prompt user to enter correct value if they get it wrong
...
while True:
    ...
    #Move to Temperature.py
        temperature(ratio_fit,var_fit,beta,'FALSE')
